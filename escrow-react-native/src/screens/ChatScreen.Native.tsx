      ...
      >
        <View style="max-w-md mx-auto">
          <View style="flex items-center gap-3 mb-3">
            ...
            <Avatar>
              <AvatarFallback style={`${isDark ? 'bg-blue-900/30 text-blue-300' : 'bg-blue-100 text-[#043b69]'}`}>
                {chat?.avatar || "AS"}
              </AvatarFallback>
            </Avatar>
            <View style="flex-1">
              ...
              <View style="flex gap-1">
                {[0, 1, 2].map((i) => (
                  <Animated.View
                    ...
                    style={`w-2 h-2 ${isDark ? 'bg-gray-500' : 'bg-gray-400'} rounded-full`}
                  />
                ))}
              </View>
            </View>
      ...
      >
        <View style="max-w-md mx-auto flex items-center gap-2">
          <motion.button
            ...
            style={`p-2 rounded-full ${isDark ? 'hover:bg-gray-700' : 'hover:bg-gray-100'}`}
          >
            <Paperclip style={`w-5 h-5 ${isDark ? 'text-gray-400' : 'text-gray-500'}`} />
          </motion.button>
          <Input
            value={message}
            ...
            style={`flex-1 ${isDark ? 'bg-gray-700 border-gray-600 text-white' : ''}`}
          />
          <Animated.View whileTap={{ scale: 0.9 }}>
            <Button
              onPress={handleSend}
