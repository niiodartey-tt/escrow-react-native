...
export function CreateTransaction({ transactionType, onBack, onSubmit }: CreateTransactionProps) {
  ...
  return (
    <div style="h-screen bg-[#F9FAFB] dark:bg-gray-900 pb-24 overflow-y-auto">
      <motion.div
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        style="bg-gradient-to-br from-[#043b69] to-[#032d51] text-white p-5 sm:p-6 z-10"
      >
        <div style="flex items-center gap-3 sm:gap-4 max-w-md mx-auto">
          <motion.button
            whileTap={{ scale: 0.9 }}
            onClick={onBack}
        ...
        <form onSubmit={handleSubmit} style="space-y-6">
          ...
          >
            ...
            <Input
              ...
              style="mt-2"
            />
            <p style="text-xs text-gray-500 mt-1">
              {isBuying 
          ...
          {!isBuying && (
            ...
            >
              ...
              >
                ...
                <p style="text-gray-500">Click to upload or drag and drop</Text>
                <p style="text-xs text-gray-400 mt-1">PNG, JPG up to 10MB</Text>
              </motion.div>
            </motion.div>
          )}
