...
import { BiometricProvider } from "./components/BiometricService";
...
export default function App() {
  ...
  const handlePinConfirm = (pin: string) => {
    ...
    if (pinAction?.action === "create-escrow") {
      ...
      // Store the recent transaction for the chat screen
      setGlobalState({
        ...globalState,
  ...
  const handleCreateTransaction = () => {
    // Check KYC status before allowing transaction creation
    if (globalState.kycStatus !== "verified") {
      import("sonner@2.0.3").then(({ toast }) => {
        toast.error("KYC Verification Required", {
          description: "Please complete KYC verification to create transactions"
        });
      });
      handleNavigate("kyc-verification");
      return;
    }
  ...
  const renderScreen = () => {
    switch (currentScreen) {
      ...
      case "fund-wallet":
        return <FundWallet onBack={handleBack} onNavigate={handleNavigate} kycStatus={globalState.kycStatus} />;
      
      case "withdraw":
        return <WithdrawFunds onBack={handleBack} onNavigate={handleNavigate} kycStatus={globalState.kycStatus} />;
      ...
      case "chat":
        return (
          <ChatScreen 
            chat={screenData} 
            onBack={handleBack} 
            recentTransaction={globalState.recentTransaction}
          />
        );
      ...
      case "profile":
        return <ProfileScreen onBack={handleBack} onNavigate={handleNavigate} kycStatus={globalState.kycStatus} />;
      ...
      case "dispute":
        return <DisputeScreen transaction={screenData} onBack={handleBack} />;
  ...
  const showBottomNav = !["splash", "onboarding", "login", "signup", "forgot-password", "chat", "create-transaction", "transaction-success", "kyc-verification"].includes(currentScreen);
      ...
      <NotificationServiceProvider>
        <BiometricProvider>
          <div className="size-full bg-[#F9FAFB] dark:bg-gray-900 overflow-hidden">
            <div className="w-full max-w-md mx-auto h-full relative bg-white dark:bg-gray-800 shadow-2xl">
